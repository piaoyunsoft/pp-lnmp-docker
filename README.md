...
├── adminer/                  # Adminer (按需启)
│
├── nginx-gateway/            # 统一入口 + acme.sh sidecar
│   ├── conf/
...
| 8.0 ~ 8.4 | alpine | 8x | alpine | **JIT 256M**, 现代化默认 |
...
- **PHP-FPM**: opcache + JIT (8.x)、dynamic pm、process_idle_timeout、慢日志、status/ping、tmpfs `/tmp`
- **MySQL**: InnoDB buffer 可调、O_DIRECT、binlog ROW、慢查询、命令行参数（绕 my.cnf Windows 权限坑）
...
3. `php/conf/8x/www.conf` `pm.max_children` 按 CPU 调
4. `opcache.validate_timestamps=0` 不变（性能最优）
...
| 大数据导入超时 | 默认还行, 老代码也许卡 | `request_terminate_timeout=0` 已设 |
